...
async def get_user_stats(
    ...
    # Items read
    items_read = base_query.filter(ContentItem.is_read == True).count()

    # Items unread
    items_unread = base_query.filter(
        ContentItem.is_read == False,
        ContentItem.is_archived == False
    ).count()

    # Items archived
    items_archived = base_query.filter(ContentItem.is_archived == True).count()

    # Total reading time (sum of reading_time_minutes for all items)
    total_reading_time = db.query(func.sum(ContentItem.reading_time_minutes))\
    ...
    read_reading_time = db.query(func.sum(ContentItem.reading_time_minutes))\
        .filter(ContentItem.user_id == current_user.id)\
        .filter(ContentItem.deleted_at.is_(None))\
        .filter(ContentItem.is_read == True)\
        .filter(ContentItem.reading_time_minutes.isnot(None))\
        .scalar() or 0
